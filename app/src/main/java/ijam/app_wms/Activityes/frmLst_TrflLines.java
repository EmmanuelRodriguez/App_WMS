...
import ijam.app_wms.Adapters.adpTrflInv_Lines;
...
public class frmLst_TrflLines extends AppCompatActivity {
    ListView oLstLines;
    ProgressDialog oPrgsDialog = null;
    adpTrflInv_Lines oAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_frmlst_trfllines);
        ...
        oLstLines =(ListView)findViewById(R.id.list_Trfl_Lines);
    ...
    public void CargaLineas(String sDocEnt){
        ...
        try{
            ...
            if (i>0){
                ...
                alstLineas= oCnt_Lineas.Reupera_Trlf_Lineas(oConn);
            }
            else{
    ...
    public void Llena_OnLineas(final ArrayList<clsCntInv_Lines> oRec) {
        try
        {
            oAdapter = new adpTrflInv_Lines(this, getOnLines(oRec)  );
            oLstLines.setAdapter(oAdapter);
            oLstLines.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    clsDatos_OnLines oDatos = (clsDatos_OnLines)((adpTrflInv_Lines)parent.getAdapter()).getItem(position);
                    Intent oAddOnHead = new Intent(frmLst_TrflLines.this, frmAdd_TrflLines.class);
        ...
        try {
            for (int i = 0; i < oResp.size(); i++) {
                if (Double.parseDouble(oResp.get(i).getsQuantity()) > 0) {
                    d = new clsDatos_OnLines(R.drawable.itm, R.drawable.btncircle_verde, oResp.get(i).getsDocEntry(), oResp.get(i).getiLineNum(), oResp.get(i).getsItemCode(), oResp.get(i).getsCodeBars(), oResp.get(i).getsItemName(), oResp.get(i).getsQuantity(), oResp.get(i).getsTypeCount());
                } else {
                    d = new clsDatos_OnLines(R.drawable.itm, R.drawable.btncircle_rojo, oResp.get(i).getsDocEntry(), oResp.get(i).getiLineNum(), oResp.get(i).getsItemCode(), oResp.get(i).getsCodeBars(), oResp.get(i).getsItemName(), oResp.get(i).getsQuantity(), oResp.get(i).getsTypeCount());
                }
